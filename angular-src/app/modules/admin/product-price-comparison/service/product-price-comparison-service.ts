...
const GET_PRODUCT_PRICE_COMPARISION_URL = '/api/bestproducts/search';
const GET_COUNT_PRODUCT_PRICE_COMPARISION_URL = '/api/bestproducts/count';
const GET_HISTORY_DETAILS = "/api/bestproducts/history";
const GET_EXPORT_EXCEL_PRODUCT_PRICE = "api/bestproducts/exporting.xlsx";
const GET_CATEGORY_DEPTH_FILTER = "/api/categorymappers/getDepthCategories";

@Injectable()
export class ProductPriceComparisonService {
    ...
    onSearchProductHistoryMatching(filter: ProductPriceComparisonFilter, pageRequest: PageRequest): Observable<any> {
        console.log("Search...");

        return this.apiService.getRaw(GET_PRODUCT_PRICE_COMPARISION_URL, this.buildParameter(filter, pageRequest)).map(resp => {
            return resp;
        }).catch(error => Observable.throw(error))
    }

    onGetTotalProductHistoryMatching(filter: ProductPriceComparisonFilter): Observable<any> {
        console.log("Counting...");
        return this.apiService.getRaw(GET_COUNT_PRODUCT_PRICE_COMPARISION_URL, this.buildParameter(filter, null)).map(resp => {
            return resp;
        }).catch(error => Observable.throw(error))
    }

    onExportExcelProductHistoryMatching(filter: ProductPriceComparisonFilter): void{
        console.log("Export excel...");
        let url = GET_EXPORT_EXCEL_PRODUCT_PRICE + '?' +this.buildParameter(filter, null).toString();
        console.info(url);
    ...
    onGetHistoryDetails(filter: ProductPriceComparisonFilter, coId: string, productId:  string) {
        let searchParams: URLSearchParams = new URLSearchParams();
        ...
        return this.apiService.getRaw(GET_HISTORY_DETAILS, searchParams).map(resp => {
            return resp;
        }).catch(error => Observable.throw(error))
