...
import {DetailedTeamActionDto, DetailedTeamActionServerResponse, DetailedTeamRow} from "../models/team-action-dto";
...
const PAGE_ID = "DETAILED_TEAM_MONITOR";
const PAGE_SUB_ID = "DETAILED_TEAM_PER_DAY_MONITOR";
const STATISTIC_HEADER = "누적접속자수";
@Component({
    selector: 'detailed-team-monitor',
    templateUrl: './detailed-team-monitor.component.html',
    styleUrls: ['./detailed-team-monitor.component.css']
})

export class DetailedTeamMonitorComponent implements OnInit {
    ...
    auditActionBindedData: DetailedTeamRow[] = [];
    auditActionPagging: DetailedTeamRow[] = [];

    subAuditActionDto: AuditActionDto = new AuditActionDto();
    subAuditActionBindedData: DetailedTeamRow[] = [];
    subAuditActionPagging: DetailedTeamRow[] = [];
    ...
    ngOnInit() {
        ...
        this.filter.unbind();
        //this.onLoadDepartments();  --Hardcode
    }
    loadData() {
        this.blockUI.start();
        this.filter.typeSearch = PAGE_ID;
        this.auditActionService.loadData(this.filter, this.pagination.pageRequest).subscribe(serverResponse => {
            switch (serverResponse.httpStatus) {
                ...
                case 'OK': {
                    let response = serverResponse as DetailedTeamActionServerResponse;
                    if (response != null && response != undefined) {
                        ...
                        this.auditActionDto = {};
                        this.auditActionBindedData.push(new DetailedTeamRow());
            ...
            switch (serverResponse.httpStatus) {
                ...
                case 'OK': {
                    ...
                    if (response != null && response != undefined) {
                        ...
                        this.subAuditActionBindedData.push(new DetailedTeamRow());
    ...
    buildTableHeader(): void {
        ...
        settingsProductSearch['searchPeriod'] = {
            ...
            width: '200px',
            align: 'center',
            render: (data: { item: DetailedTeamRow, value: any }, settings: object) => {
        ...
        settingsProductSearch['teamName'] = {
            ...
            align: 'center',
            render: (data: { item: DetailedTeamRow, value: any }, settings: object) => {
                if (data.item['teamName'] === STATISTIC_HEADER) {
                    return '<span style="color:blue">' + data.item['teamName'] + '</span>';
                } else {
                    return data.item['teamName'];
                }
            }
        };
        settingsProductSearch['userCount'] = {
            title: 'monitorUserActivities.column.userCount',
            width: '100px',
            align: 'right',
            render: (data: { item: DetailedTeamRow, value: any }, settings: object) => {
                if (data.item['teamName'] === STATISTIC_HEADER) {
                    return '<span style="color:blue">' + data.item['userCount'] + '</span>';
                } else {
                    return data.item['userCount'];
                }
            }
        };
        settingsProductSearch['menuName'] = {
            ...
            render: (data: { item: DetailedTeamRow, value: any }, settings: object) => {
                if (data.item['teamName'] === STATISTIC_HEADER) {
                    return '<span style="color:blue">' + data.item['menuName'] + '</span>';
                } else {
    ...
    convertAPIDataToTableRowData(response:DetailedTeamActionServerResponse): void {
        ...
        if (listObjs == null || listObjs.length == 0) {
            ...
            listObjs.forEach(item => {
                let competitor: DetailedTeamActionDto = item;
                let row = new DetailedTeamRow();
                if (competitor != null) {
                    ...
                    row.userCount = competitor.userCount;
    ...
    convertAPIDataToSubTableRowData(response:DetailedTeamActionServerResponse): void {
        let listObjs = response.data ;
        if (listObjs == null || listObjs.length == 0) {
            this.subAuditActionBindedData = [];
        } else {
                ...
                switch (serverResponse.httpStatus) {
                    ...
                    case 'OK': {
                        ...
                        departmentsInDetails.data.forEach(gr => {
                            ...
                            */
                            gr.childs.forEach(team => {
                                if (gr.cateNo != team.cateNo) this.teams.push({name: team.name, id: team.name});
                            });
                        });
                    }
                }
    ...
    onCheckedShowingAnalysis(event) {
        this.filter.isShowAnalysis=!this.filter.isShowAnalysis;
    }
}
